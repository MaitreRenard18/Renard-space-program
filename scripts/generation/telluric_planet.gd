...
func place_scene(scene: PackedScene, theta: float) -> void:
	var instance = scene.instantiate()
	instance.position = Vector2(cos(theta), sin(theta)) * (get_terrain_height(theta))
	instance.rotation = get_terrain_angle(theta)
	instance.z_index = -1
	add_child(instance)
...
func _ready():
	...
	planet_renderer.material.set_shader_parameter("noise_strength", noise_strength)
